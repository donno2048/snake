...
- An empty C program with size optimization turned on, no standard startup files, no standard libraries, no symbol information, and no code at all, compiled with `gcc -Os -nostartfiles -nodefaultlibs -lc --entry 0 -Wl,--strip-all -xc /dev/null` on {platform} is {optimized_size} bytes.
