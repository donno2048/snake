...
.input:                    ; handle keyboard input
    ...
    lea ax, [di+bx+0x2]    ; AX = position + offset + 2, for checking horizontal edge later
    ...
    and ah, ah             ;   check if remainder in AH is 0, which is the case only when head is on the left edge and movement was right (offset 2) or head is on the right edge and movement was left (offset -2)
