...
xchg ax, bx              ;   alternate BX between the last head position and the end of the screen
...
adc BYTE [si], 0x1F      ; set snake character
...
das                      ; we don't use AL so this "skips" the first iteration which is good, after the first iteration we jump to .wall-1 which gives us 0xD1 and since DAS is 0x2F we get the instruction `shr word [bx],1` so after enough iterations we get a wall of null bytes
sub bx, dx               ;   go one line and 0x2000 bytes backwards (the added bytes wrap nicely as 0x10000%0x2000=0)
jnz .wall-1              ; jump to draw the next wall, jumping using DX and not 0x50 makes this a delay loop
