...
start:                   ; reset game
    ...
    mov bx, [si]         ;   reset head position, SI always points to a valid screen position containing 0x720 after setting video mode, also SBB when jumping to .food-1
.food:                   ; create new food item, always jumping to .food-1 for randomizing with SBB
    lahf                 ;   read FLAGS into AH for randomization after restart and `SBB AL, 0x9F` for the next food generations
    and si, ax           ;     mask with SI to make divisible by 4 and less than or equal to screen size
    add [si], cl         ;   place food item and check if position was empty by applying ADD with CL (assumed to be 0xFF)
.input:                  ; handle keyboard input
    mov si, 0x7D0        ; initialize SI
    js .food-1           ;     if position was occupied by snake or wall in food generation => try again, if we came from main loop SF=0
    ...
    jc .food-1           ; if food was consumed, CF=1 from RCR => generate new food
