...
; BX: position of the snake head (only every second horizontal position is ever used to compensate the speed difference between horizonal and vertical movements)
...
.food:                ; create new food item
    ...
    xchg di, ax       ;     move into DI because indirect addressing is not possible with AX, using XCHG instead of MOV saves 1 byte
    cmp [di], dh      ;   check if new food position is empty
    jne .food         ;     if not => try again
    mov [di], ch      ;   place new food item on screen
.input:               ; handle keyboard input
    ...
    add bx, ax        ; add sign extended offset to head position
    xchg ax, bx       ; swap AX and BX, now BX=offset and AX=position+offset
    add bx, ax        ; update head position all the way, now BX=position+2*offset
    cmp bx, cx        ; check if head crossed vertical edge by comparing against screen size in CX, needs to be done before DIV to avoid exception when BX<0
    ja start          ;   if BX<0 or BX>CX => game over
    ...
    xor [bx], dl      ; XOR head position with snake character
    jns start         ;   if it already had snake in it, SF=0 from XOR => game over
    push bx           ; push new head position onto the stack
    ...
    xchg di, ax       ; move into DI because indirect addressing is not possible with AX, using XCHG instead of MOV saves 1 byte
    mov [di], dh      ; clear old tail position on screen
